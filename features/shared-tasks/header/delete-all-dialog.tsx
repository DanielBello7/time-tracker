...
import deleteAllSharedTasks from "@/apis/delete-all-shared-tasks";

export default function DeleteAllSharedTasksDialog() {
  const { _id } = useAppSelector((state) => state.user.user);
  const dispatch = useAppDispatch();

  const handleDeleteAll = () => {
    deleteAllSharedTasks(_id)
      .then(() => {
        dispatch(resetTasks());
        toast("All Shared Task Deleted");
      })
      .catch((error) => {
    ...
    <AlertDialogContent className="w-full md:w-[320px] p-0">
      <AlertDialogHeader className="p-5">
        ...
        <AlertDialogDescription className="text-center">
          This action cannot be undone. This will permanently delete all shared tasks from your
